...
describe('ShopUI', () => {
    ...
    test('updateShopUI reflects selected piece', () => {
        ...
        ShopUI.updateShopUI(game);
        expect(document.getElementById('selected-piece-display').textContent).toContain('Piece(p, white)');

        game.selectedShopPiece = null;
        ShopUI.updateShopUI(game);
        expect(document.getElementById('selected-piece-display').textContent).toBe('Wähle eine Figur zum Kaufen');
    });
});
