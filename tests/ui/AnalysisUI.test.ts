...
describe('AnalysisUI', () => {
  ...
  test('showAnalysisPrompt should call UI.showModal', async () => {
    ...
    // Wait for dynamic import / async logic
  ...
  test('startFullAnalysis should handle empty move history', async () => {
    ...
    expect((analysisUI as any).isAnalyzing).toBe(false);
  });
  ...
  test('collectBoardStates should work backwards from current board', () => {
    ...
    vi.spyOn(analysisUI as any, 'undoMoveOnBoard').mockImplementation(function (board: any) {
      board[0][0] = 'initial';
    });
  ...
  test('undoMoveOnBoard should handle normal move', () => {
    ...
    (analysisUI as any).undoMoveOnBoard(board, move);
