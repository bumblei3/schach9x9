...
describe('GameStateManager', () => {
    ...
    describe('undoMove', () => {
        ...
        test('should set lastMoveHighlight after undo if history remains', () => {
            game.moveHistory = [
                ...
                { from: { r: 8, c: 4 }, to: { r: 6, c: 4 }, piece: { type: 'p' }, positionHistoryLength: 0 }
            ];
            game.board[6][4] = { type: 'p', color: 'white' };
            GameStateManager.undoMove(game, moveController);
    ...
    describe('Replay Mode', () => {
        ...
        test('reconstructBoardAtMove with savedGameState', () => {
            game.savedGameState = { board: Array(9).fill(null).map(() => Array(9).fill(null)) };
            game.moveHistory = [
    ...
    describe('Save/Load', () => {
        ...
        test('should load with draw offer', () => {
            ...
            expect(document.getElementById('draw-offer-overlay').classList.contains('hidden')).toBe(false);
            expect(document.getElementById('draw-offer-message').textContent).toContain('Schwarz bietet Remis an');
        });
        ...
        test('should load with white draw offer', () => {
            ...
            GameStateManager.loadGame(game);
            expect(document.getElementById('draw-offer-message').textContent).toContain('Weiß bietet Remis an');
        });
        ...
        test('should load in play phase and update UI', () => {
            ...
            GameStateManager.loadGame(game);
            expect(document.getElementById('move-history-panel').classList.contains('hidden')).toBe(false);
