...
export function runRulesFuzzTest(iterations: number = 1000) {
  ...
  const resetBoard = () => {
    ...
    // Ensure kings exist
    if (!board.includes(PIECE_KING | COLOR_WHITE))
      board[Math.floor(Math.random() * 9) + 72] = PIECE_KING | COLOR_WHITE;
    if (!board.includes(PIECE_KING | COLOR_BLACK))
      board[Math.floor(Math.random() * 9)] = PIECE_KING | COLOR_BLACK;
  };
