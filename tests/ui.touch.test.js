...
describe('BoardRenderer Touch Interactions', () => {
    ...
    test('should initiate drag on touchstart for valid piece', () => {
        ...
        const clones = Array.from(document.body.children).filter(el =>
            el.style && el.style.position === 'fixed' && el.classList.contains('piece-svg')
        );
    ...
    test('should move dragged element on touchmove', () => {
        ...
        const clone = Array.from(document.body.children).find(el =>
            el.style.position === 'fixed' && el.classList.contains('piece-svg')
        );
