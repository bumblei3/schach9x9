...
vi.mock('../js/ui.js', () => ({
  ...
  showPromotionUI: vi.fn((g, r, c, _col, _mr, cb) => {
    // Manually mutate board for mock promotion to execute
...
describe('MoveExecutor Integration Tests', () => {
  ...
  beforeEach(() => {
    ...
    document.getElementById = vi.fn((id: string) => {
      if (id === 'winner-text') return mockElement as any;
      if (id === 'game-log') return mockElement as any;
  ...
  test('Capture: Updates captured pieces logic', async () => {
    game.board[6][4] = { type: 'r', color: 'white', hasMoved: true };
    game.board[5][4] = { type: 'p', color: 'black', hasMoved: true };
  ...
  test('Check Detection: Integration with MoveValidator', async () => {
    ...
    game.board[0][4] = { type: 'k', color: 'black', hasMoved: true };
    game.board[8][4] = { type: 'r', color: 'white', hasMoved: true }; // On same file
    ...
    game.board[0][4] = null;
    game.board[3][3] = { type: 'k', color: 'black', hasMoved: true };

    // White Rook at 3,0.
    game.board[3][0] = { type: 'r', color: 'white', hasMoved: true };
  ...
  test('Checkmate: Real Checkmate Scenario', async () => {
    ...
    game.board[1][8] = { type: 'r', color: 'white', hasMoved: true };

    // White Rook 2 moves to row 0 to mate.
    game.board[6][8] = { type: 'r', color: 'white', hasMoved: true };
  ...
  test('Insufficient Material: Real Logic', async () => {
    ...
    document.getElementById = vi.fn((id: string) => {
      if (id === 'winner-text') return winnerText as any;
      if (id === 'game-log') return logMock as any;
    ...
    game.board = Array(9)
      ...
      .map(() => Array(9).fill(null)) as any;
    game.board[0][4] = { type: 'k', color: 'black', hasMoved: true };
    game.board[8][4] = { type: 'k', color: 'white', hasMoved: true };
  ...
  test('3D Battle: Integration with window.battleChess3D', async () => {
    ...
    game.board[6][0] = { type: 'r', color: 'white', hasMoved: true };
    game.board[5][0] = { type: 'p', color: 'black', hasMoved: true };
    await MoveExecutor.executeMove(game, moveController, { r: 6, c: 0 }, { r: 5, c: 0 });
    expect((window as any).battleChess3D.playBattleSequence).toHaveBeenCalled();
  });
  ...
  test('Puzzle Mode: Correct Move', async () => {
    game.mode = 'puzzle';
    game.puzzleState = { currentMoveIndex: 0, puzzleId: 'test-puzzle' } as any;
    game.board[6][4] = { type: 'p', color: 'white', hasMoved: true };
    (puzzleManager.checkMove as Mock).mockReturnValue('correct');
  ...
  test('Puzzle Mode: Solved', async () => {
    game.mode = 'puzzle';
    game.board[6][4] = { type: 'p', color: 'white', hasMoved: true };
    (puzzleManager.checkMove as Mock).mockReturnValue('solved');
  ...
  test('Puzzle Wrong: Triggers error UI and sound', async () => {
    ...
    (puzzleManager.checkMove as Mock).mockReturnValue('wrong');
  ...
  test('Auto-save: Triggers saveGame after a move', async () => {
    ...
    // Setup
    game.moveHistory = [1, 2, 3, 4] as any; // 4 moves already
