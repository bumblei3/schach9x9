...
jest.unstable_mockModule('../js/gameEngine.js', () => ({
    Game: class { constructor() { this.board = []; this.capturedPieces = { white: [], black: [] }; } },
...
describe('Campaign Integration', () => {
    ...
    test('startCampaignLevel should initialize game with level configuration', () => {
        ...
        expect(mockUI.showModal).toHaveBeenCalledWith(level.title, expect.any(String), expect.any(Array));
    });
    ...
    test('handleGameEnd should complete level on victory', () => {
        ...
        expect(localStorage.setItem).toHaveBeenCalledWith('schach9x9_campaign_progress', expect.stringContaining(levelId));
    });
