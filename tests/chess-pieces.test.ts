...
describe('Chess Pieces', () => {
  describe('PIECE_SETS', () => {
    ...
    test('each skin should have all piece types', () => {
      ...
      Object.keys(PIECE_SETS).forEach(skinName => {
        const skin = PIECE_SETS[skinName as keyof typeof PIECE_SETS];
    ...
    test('each piece should be a non-empty string', () => {
      Object.values(PIECE_SETS).forEach(skin => {
        Object.values(skin.white).forEach((svg: any) => {
          expect(typeof svg).toBe('string');
          expect(svg.length).toBeGreaterThan(0);
        });
        Object.values(skin.black).forEach((svg: any) => {
          expect(typeof svg).toBe('string');
          expect(svg.length).toBeGreaterThan(0);
        });
      });
  ...
  describe('setPieceSkin', () => {
    ...
    test('should switch to modern skin', () => {
      setPieceSkin('modern');
      expect((window as any).PIECE_SVGS.white.k).toBe(PIECE_SETS.modern.white.k);
      expect((window as any).PIECE_SVGS.black.q).toBe(PIECE_SETS.modern.black.q);
    });

    test('should switch to pixel skin', () => {
      setPieceSkin('pixel');
      expect((window as any).PIECE_SVGS.white.r).toBe(PIECE_SETS.pixel.white.r);
      expect((window as any).PIECE_SVGS.black.n).toBe(PIECE_SETS.pixel.black.n);
    });

    test('should switch to infernale skin', () => {
      setPieceSkin('infernale');
      expect((window as any).PIECE_SVGS.white.b).toBe(PIECE_SETS.infernale.white.b);
      expect((window as any).PIECE_SVGS.black.p).toBe(PIECE_SETS.infernale.black.p);
    });

    test('should switch to wood skin', () => {
      setPieceSkin('wood');
      expect((window as any).PIECE_SVGS.white.a).toBe(PIECE_SETS.wood.white.a);
      expect((window as any).PIECE_SVGS.black.c).toBe(PIECE_SETS.wood.black.c);
    });

    test('should switch to neon skin', () => {
      setPieceSkin('neon');
      expect((window as any).PIECE_SVGS.white.e).toBe(PIECE_SETS.neon.white.e);
      expect((window as any).PIECE_SVGS.black.k).toBe(PIECE_SETS.neon.black.k);
    });

    test('should switch to minimalist skin', () => {
      setPieceSkin('minimalist');
      expect((window as any).PIECE_SVGS.white.q).toBe(PIECE_SETS.minimalist.white.q);
      expect((window as any).PIECE_SVGS.black.r).toBe(PIECE_SETS.minimalist.black.r);
    });
    ...
    test('should handle switching between skins multiple times', () => {
      ...
      setPieceSkin('pixel');
      expect((window as any).PIECE_SVGS.white.k).toBe(PIECE_SETS.pixel.white.k);

      setPieceSkin('classic');
      expect((window as any).PIECE_SVGS.white.k).toBe(PIECE_SETS.classic.white.k);
    });
    ...
    test('should apply skin to all piece types', () => {
      ...
      pieceTypes.forEach(type => {
        expect((window as any).PIECE_SVGS.white[type]).toBe(PIECE_SETS.infernale.white[type]);
        expect((window as any).PIECE_SVGS.black[type]).toBe(PIECE_SETS.infernale.black[type]);
      });
    });
  });
  ...
  describe('PIECE_SVGS global', () => {
    test('should be accessible via window.PIECE_SVGS', () => {
      expect((window as any).PIECE_SVGS).toBeDefined();
      expect((window as any).PIECE_SVGS.white).toBeDefined();
      expect((window as any).PIECE_SVGS.black).toBeDefined();
    });
  ...
  describe('setPieceSkin with invalid skin', () => {
    ...
    test('should not change skin when given invalid name', () => {
      setPieceSkin('pixel');
      const beforeSkin = (window as any).PIECE_SVGS.white.k;

      setPieceSkin('nonexistent');

      expect((window as any).PIECE_SVGS.white.k).toBe(beforeSkin);
    });
  });
});
