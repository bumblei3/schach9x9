...
describe('AI Engine - Evaluation Boost', () => {
  ...
  test('King Safety: King Zone Attacks', () => {
    ...
    const threatenedScore = evaluatePosition(board, 'white');
    ...
    const highlyThreatenedScore = evaluatePosition(board, 'white');
    expect(highlyThreatenedScore).toBeLessThan(threatenedScore);
  });

  test('Supported Passed Pawns bonus', () => {
    // White passed pawn
    board[4][4] = { type: 'p', color: 'white' };
    // No opponent pawns in front or on adjacent columns

    const isolatedPassedScore = evaluatePosition(board, 'white');

    // Supporting pawn
    board[5][3] = { type: 'p', color: 'white' };
    const supportedPassedScore = evaluatePosition(board, 'white');
