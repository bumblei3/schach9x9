...
import { getBestMoveWasm, getWasmNodesEvaluated, resetWasmNodesEvaluated } from './ai/wasmBridge.js';
    ...
    if (typeof Worker !== 'undefined' && typeof window !== 'undefined') {
        try {
            ...
            if (result && result.move) {
                ...
                return {
                    ...result,
                    move: convertMoveToResult(result.move as unknown as { from: number; to: number; promotion?: number }),
                };
            }
        } catch (err) {
...
export function isSquareAttacked(uiBoard: UiBoard, r: number, c: number, turnColor: Player): boolean {
    const board = convertBoardToInt(uiBoard);
...
// Helpers for tests
export function makeMove(uiBoard: UiBoard, uiMove: { from: Square; to: Square } | null): UndoInfo | null {
