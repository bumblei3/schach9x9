...
export function buildCenteredPST(
  ...
  for (let r = 0; r < 9; r++) {
    for (let c = 0; c < 9; c++) {
      const d = Math.max(Math.abs(r - PSQT_CENTER.r), Math.abs(c - PSQT_CENTER.c)); // 0..4
      const f = Math.pow(1 - d / 4, exponent);
      t.push(Math.round(edgeBonus + (maxBonus - edgeBonus) * f));
    }
  }
  return t;
}
...
const KING_MIDGAME_TABLE = [
  ...
  0, 20, 20, 0, 0, 0, 0, 0, 20, 20, 20, 30, 10, 0, 0, 0, 10, 30, 20,
];
...
const PASSED_PAWN_BONUS = [0, 150, 100, 60, 35, 20, 10, 5, 0];
...
export function evaluate(
  b: IntBoard,
  c: number,
  evalConfig: EvalConfig = { personality: 'NORMAL' }
): number {
  let mgScore = 0;
  let egScore = 0;
  let phase = 0;
  ...
  for (let i = 0; i < SQUARE_COUNT; i++) {
    ...
    if (type !== PIECE_PAWN && type !== PIECE_KING) {
      let mobility = 0;
      const directions =
        type === PIECE_BISHOP || type === PIECE_ARCHBISHOP
          ? BISHOP_DIRS
