...
import type { AIDifficulty, GameMode, TimeControl } from './config.js';
...
export class GameController {
    ...
    async handleCellClick(r: number, c: number): Promise<void> {
        ...
        } else if (this.game.phase === (PHASES.PLAY as any) || (this.game.phase as any) === 'ANALYSIS') {
    ...
    declineDraw(): void {
        ...
        const decliningColor =
            this.game.turn === this.game.drawOfferedBy
                ? (this.game.turn === 'white' ? 'black' : 'white')
                : this.game.turn;
    ...
    handleGameEnd(result: string, winnerColor: Player): void {
        ...
        if (this.game.campaignMode && result === 'win' && winnerColor === this.game.playerColor) {
            if (this.game.currentLevelId) {
                ...
                setTimeout(() => {
                    (UI as any).showCampaignVictoryModal(level.title, 3 /* Needs actual star calculation return */, [
                        {
