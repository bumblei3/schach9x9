...
import { Game, type GameMode } from '../gameEngine.js';
...
interface BookData {
  ...
  metadata: { version: string; generatedAt: string; mode: string };
}
...
async function generateBook(): Promise<void> {
  ...
  const games = parser.parse(pgnContent) as unknown[];
  ...
  for (const gameData of games) {
    const gameEngine = new Game(15, mode as GameMode);

    const history = parser.replayGame((gameData as { moves: string[] }).moves, gameEngine);
