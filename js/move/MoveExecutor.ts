...
export async function executeMove(
    ...
    } else if ((moveRecord as any).specialMove && (moveRecord as any).specialMove.type === 'enPassant') {
        const capturerColor = piece.color;
    ...
    if (game.mode === 'puzzle') {
        ...
        if (result === 'wrong') {
            ...
            const puzzle = (puzzleManager as any).getPuzzle(game.puzzleState ? (game.puzzleState as any).puzzleId : '');
...
export function finishMove(game: Game): void {
    ...
    if ((window as any).battleChess3D && (window as any).battleChess3D.enabled && game.moveHistory.length > 0) {
        ...
        const captured =
            targetPiece || ((lastMove as any).specialMove && (lastMove as any).specialMove.type === 'enPassant');
        if (captured) {
            ...
            (window as any).battleChess3D.playBattleSequence(attackerData, defenderData, from, to).then(() => {
    ...
    if (game.isAI && game.turn === 'black' && game.phase === (PHASES.PLAY as any)) {
        ...
        setTimeout(() => {
            ...
            if (
                ...
                ((game as any).analysisMode || ((game as any).aiController && (game as any).aiController.analysisActive))
            ) {
