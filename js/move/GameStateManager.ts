...
export function undoMove(game: Game, moveController: MoveController): void {
    ...
    game.board[move.to.r][move.to.c] = move.captured
        ? { type: (move.captured as any).type, color: (move.captured as any).color, hasMoved: true } as any
        : null;
    ...
    UI.updateMoveHistoryUI(game);
    UI.updateStatus(game);
    game.log(`Zug ${move.piece && (move.piece as any).color === 'white' ? 'Weiß' : 'Schwarz'} zurückgenommen`);
...
export function exitReplayMode(game: Game): void {
    ...
    const undoBtn = document.getElementById('undo-btn') as HTMLButtonElement;
    if (undoBtn) undoBtn.disabled = game.moveHistory.length === 0 || game.phase !== (PHASES as any).PLAY;
...
export function loadGame(game: Game): boolean {
    ...
    try {
        ...
        if (game.phase === (PHASES.SETUP_WHITE_PIECES as any) || game.phase === (PHASES.SETUP_BLACK_PIECES as any)) {
