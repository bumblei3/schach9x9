...
export class SceneManager3D {
  ...
  public boardGroup: THREE.Group | null;
  ...
  getThemeColors(theme: string): { light: number; dark: number } {
  ...
  boardToWorld(row: number, col: number): { x: number; z: number } {
  ...
  setTheme(themeName: string): void {
    ...
    this.boardGroup.children.forEach(child => {
      const square = child as THREE.Mesh;
      if (square.userData && square.userData.type === 'square') {
        const isLight = square.userData.isLight;
        const mat = square.material as THREE.MeshStandardMaterial;
        mat.color.setHex(isLight ? colors.light : colors.dark);
      }
    });
  }
