 ...
 */
export async function showTutorSuggestions(game: Game, providedHints: any[] | null = null): Promise<void> {
    const g = game as any;
    ...
    hints.forEach((hint: any, index: number) => {
        ...
        suggEl.addEventListener('click', () => {
            document.querySelectorAll<HTMLElement>('.tutor-suggestion').forEach(el => (el.style.borderLeft = ''));
            const color =
