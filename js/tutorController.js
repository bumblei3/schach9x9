...
export class TutorController {
  ...
  getTutorHints() {
    ...
    if (evaluatedMoves.length > 0) {
      logger.debug(
        `Tutor: Evaluated ${evaluatedMoves.length}/${moves.length} moves, best: ${evaluatedMoves[0].notation} (${evaluatedMoves[0].score})`
      );
    }
  ...
  checkBlunder(moveRecord) {
    ...
    if (drop >= 200) {
      ...
      const analysis = this.analyzeMoveWithExplanation(
        moveRecord,
        currentEval,
